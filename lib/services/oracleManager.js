...
import { getRandomFunPhrase, getRandomKaomoji } from '@/lib/services/moeHandler'; // Asumiendo ambas existen
...
export async function queryOracle(questionText) {
  ...
  const keywords = questionText.toLowerCase().replace(/[^\w\sÁÉÍÓÚáéíóúñÑ]/gi, '').split(/\s+/).filter(kw => kw.length > 2);

  if (keywords.length === 0) {
    return `Mmm~ Tu pregunta es como una nubecita... ¡Intenta con palabritas más concretas, porfi! ☁️ ${thinkingKaomoji}`;
  }
  ...
  try {
    let result = await query(
      `SELECT text, user_first_name, created_at 
       FROM oracle_phrases
       WHERE is_decision = TRUE AND to_tsvector('spanish', text) @@ to_tsquery('spanish', $1)
       ORDER BY RANDOM()
       LIMIT 1`,
      [searchQuery]
    );
    ...
    if (result.rowCount > 0) {
      ...
      const respuestasGenericasMoe = [
        `Uguu~ El Oráculo ha buscado por todas partes, ¡pero no encuentra nada sobre eso! ${sadKaomoji} ¿Quizás con otras palabritas?`,
        `Las estrellitas no me dicen nada esta vez... Intenta preguntar de una forma más kawaii, ¿siii? ⭐ ${thinkingKaomoji}`,
        `Moe-Moe-Kyun!~ El Oráculo está recargando su energía moe... Mientras, ¿por qué no pruebas otra cosita? ⚡ ${getRandomKaomoji()}`,
        `Kyaa~ ¡Tu pregunta es un misterio! ${getRandomKaomoji()} El Oráculo necesita más pistas.`,
        `El Oráculo susurra: "No lo sé... pero seguro que ${keywords.join(' y ')} suena importante~".`
      ];
