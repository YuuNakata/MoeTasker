...
export async function completeTask(taskId, completingUserId) {
  try {
    ...
    if (parseInt(task.assigned_to_id) !== parseInt(completingUserId) /* && completingUserId !== ADMIN_ID */) {
       return `❌ Solo ${mention(task.assigned_to_name, task.assigned_to_id)} o un admin puede marcar esta tarea.(${task.assigned_to_id} !== ${completingUserId} )`;
    }
