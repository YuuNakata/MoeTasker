...
export async function assignTasks(chatId, taskDescriptions) {
  ...
  taskDescriptions.forEach((desc, index) => {
    ...
    assignmentsText += `  🔹 ${escapeMarkdownV2(desc.trim())} ➡️ [${escapeMarkdownV2(memberName)}](tg://user?id=${memberId})\n`;
  });
...
export async function completeTask(taskId, completingUserId) {
  ...
  if (task.completed) return `🤔 La tarea '${escapeMarkdownV2(task.description)}' (\`${taskId}\`) ya estaba completada.`;

  if (task.assigned_to_id !== completingUserId) {
     return `❌ Solo [${escapeMarkdownV2(task.assigned_to_name)}](tg://user?id=${task.assigned_to_id}) o un admin puede marcar esta tarea.`;
  }

  task.completed = true;
  task.completed_at = new Date().toISOString();
  saveTaskToDB(task);
  return `✅ ¡Bien hecho! Tarea '${escapeMarkdownV2(task.description)}' (\`${taskId}\`) marcada como completada por ${escapeMarkdownV2(task.assigned_to_name)}.`;
}
...
export async function getPendingTasksSummary() {
  ...
  pending.forEach(task => {
    summary += (
      `\n🔹 \`${task.id}\`: ${escapeMarkdownV2(task.description)} ` +
      `(Asignada a: [${escapeMarkdownV2(task.assigned_to_name)}](tg://user?id=${task.assigned_to_id}))` +
