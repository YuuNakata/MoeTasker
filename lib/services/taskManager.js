...
export async function getActiveTeamMembers(chatId) {
  try {
    ...
    if (!administrators || administrators.length === 0) {
      console.warn("No se pudieron obtener administradores del chat");
      return [];
    }
    ...
    console.log(
      `Miembros activos encontrados: ${activeMembers.map((m) => m.name).join(", ")}`,
    );
...
export async function assignTasks(chatId, taskDescriptions) {
  ...
  // Obtener miembros activos dinámicamente
