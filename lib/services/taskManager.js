...
// ID de Claudia para excluirla de las asignaciones
const CLAUDIA_USER_ID = parseInt(process.env.USER_ID_CLAUDIA || "0");

// Fallback: miembros estáticos del equipo (sin Claudia)
...
export async function getActiveTeamMembers(chatId) {
  try {
    ...
    // Obtener todos los miembros disponibles del chat (administradores + registrados) excepto Claudia
...
export async function getPendingTasksSummary() {
  try {
    const res = await query(
      "SELECT id, description, assigned_to_name, assigned_to_id FROM tasks WHERE completed = FALSE ORDER BY assigned_at ASC",
    );
...
export async function getTasksForDailyReminder() {
  try {
    ...
    const res = await query(
      "SELECT id, description, assigned_to_name, assigned_to_id, chat_id FROM tasks WHERE completed = FALSE ORDER BY assigned_at ASC",
    );
    return res.rows;
  } catch (error) {
    console.error(
...
export async function clearAllTasks(userId) {
  ...
  try {
    ...
    return {
      success: false,
      message: "⚠️ Ocurrió un error al intentar borrar las tareas.",
    };
  }
}
