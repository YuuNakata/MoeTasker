...
import { sendMessage, getChatAdministrators } from "@/utils/telegram";
...
// Función para obtener miembros activos del grupo dinámicamente
export async function getActiveTeamMembers(chatId) {
  try {
    ...
    // Obtener todos los administradores del chat
    const administrators = await getChatAdministrators(targetChatId);

    if (!administrators || administrators.length === 0) {
      console.warn(
        `No se pudieron obtener administradores del chat ${targetChatId}`,
      );
      return [];
    }

    // Filtrar solo usuarios humanos (no bots) y excluir a Claudia
    const activeMembers = administrators
      .filter((admin) => {
        const user = admin.user;
        return (
          !user.is_bot &&
          user.id !== CLAUDIA_USER_ID &&
          user.first_name &&
