...
/**
 * Gets commands as an array for the AI Actions system
...
export function parseCommand(text) {
  ...
  let commandPart = parts[0].substring(1); // Remove /

  // Remove @BotName if present (Telegram format: /command@BotName)
  const atIndex = commandPart.indexOf("@");
  if (atIndex !== -1) {
    commandPart = commandPart.substring(0, atIndex);
  }

  // Handle /complete_TASKID or /completar_TASKID format
  let args = parts.slice(1).join(" ");
  if (commandPart.includes("_")) {
    const underscoreIndex = commandPart.indexOf("_");
    const baseCommand = commandPart.substring(0, underscoreIndex);
    const taskId = commandPart.substring(underscoreIndex + 1);

    // Check if this is a valid command with underscore format
    const command = getCommand(baseCommand.toLowerCase());
    if (
      command &&
      (baseCommand === "complete" || baseCommand === "completar")
    ) {
      return {
        name: baseCommand.toLowerCase(),
        command,
        rawArgs: taskId,
        fullText: text,
      };
