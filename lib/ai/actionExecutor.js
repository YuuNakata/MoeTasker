...
import { validateSQLQuery, sanitizeParams, validateResultLimits } from './sqlValidator';
...
async function executeAction(action, context) {
  ...
  throw new Error(`Action failed after ${RETRY_CONFIG.maxRetries} attempts: ${lastError.message}`);
}
...
async function executeSQLAction(data, context) {
  ...
  const { query: sanitizedQuery, params: sanitizedParams, operation } = validation.sanitized;
