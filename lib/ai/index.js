...
export async function processAIResponseWithActions(
  ...
  processed.actions.forEach((action, idx) => {
    console.log(`   ${idx + 1}. [${action.type}]`,
      action.type === 'sql' ? action.data.query.substring(0, 60) + '...' : action.data.action || 'operation'
    );
  });
  ...
  while (attempt < maxRetries) {
    try {
      ...
      console.log(`   Completed: ${executionResults.successCount}/${executionResults.totalActions}`);
