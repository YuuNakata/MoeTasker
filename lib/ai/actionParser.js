  ...
  switch (data.action) {
    ...
    case "editMessage":
      ...
      if (!data.message_id && !data.inline_message_id) {
        return {
          valid: false,
          error: "editMessage requires message_id or inline_message_id",
        };
      }
      break;
    ...
    case "sendSticker":
      ...
      if (!data.sticker) {
        return {
          valid: false,
          error: "sendSticker requires sticker (file_id or URL)",
        };
      }
      break;
  }
...
function validateMemoryAction(data) {
  ...
  if (["save", "get", "delete"].includes(data.operation) && !data.key) {
    return {
      valid: false,
      error: `${data.operation} operation requires a key`,
    };
  }
...
export function replaceVariables(data, context = {}) {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (
      ...
      // Try to get the value from context with multiple possible keys
      let replacedValue = value;
      if (context[varName] !== undefined) {
        replacedValue = context[varName];
      } else if (varName === "chat_id" && context.chatId !== undefined) {
        replacedValue = context.chatId;
      } else if (varName === "chatId" && context.chat_id !== undefined) {
        replacedValue = context.chat_id;
      } else if (varName === "user_id" && context.userId !== undefined) {
        replacedValue = context.userId;
      } else if (varName === "userId" && context.user_id !== undefined) {
        replacedValue = context.user_id;
      }
