...
export function validateSQLQuery(query, params = [], context = {}) {
  ...
  const multiTenancyCheck = validateMultiTenancy(normalizedQuery, tables, context);
...
function checkParameterUsage(query, params) {
  ...
  for (const pattern of suspiciousPatterns) {
    if (pattern.test(query)) {
      return {
        valid: false,
        error: 'Possible string concatenation detected. Use parameterized queries ($1, $2, etc.)'
      };
    }
  }
...
function validateMultiTenancy(normalizedQuery, tables, context) {
  ...
  const multiTenantTables = ['tasks', 'team_members', 'oracle_memories', 'group_config', 'ai_memory'];

  const requiresChatId = tables.some(table => multiTenantTables.includes(table));
