...
import { getAiResponse } from "@/pages/api/chat";

/**
 * Limpia y mejora la descripción de una tarea usando IA
 */
async function cleanTaskDescription(rawDescription, chatId) {
  try {
    const prompt = `Convierte esta descripción de tarea en una descripción clara y concisa (máximo 10 palabras).
Solo responde con la descripción mejorada, sin explicaciones adicionales.
Descripción original: "${rawDescription}"`;

    const cleanedDescription = await getAiResponse(
      [{ role: "user", content: prompt }],
      null,
      chatId,
    );

    // Si la IA devuelve algo válido y no muy largo, usarlo
    if (cleanedDescription && cleanedDescription.length < 100) {
      return cleanedDescription.trim();
    }

    // Si falla o es muy largo, devolver el original limpio
...
export async function handleAssign(context, params) {
  ...
  try {
    ...
    // Limpiar y mejorar descripciones de tareas usando IA
    console.log("Limpiando descripciones de tareas con IA...");
    const cleanedTasks = await Promise.all(
      tasks.map((task) => cleanTaskDescription(task, chatId)),
    );
    console.log(
      "Tareas limpiadas:",
      cleanedTasks.map((t, i) => `"${tasks[i]}" → "${t}"`),
    );
    ...
    // Convertir miembros al formato esperado por assignTasks
    ...
    console.log(
      `Asignando tareas a miembros del chat ${chatId}: ${formattedMembers.map((m) => m.name).join(", ")}`,
    );

    // Asignar las tareas limpias pasando los miembros
