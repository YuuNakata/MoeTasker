...
import { registerAIActionsCommands } from "./aiActionsCommands";
...
export function registerAllCommands() {
  ...
  registerCommand({
    name: "addMember",
    description:
      "Add a new member to the team. Reply to their message or mention them with @username.",
    parameters: [
      {
        name: "args",
        type: "string",
    ...
    requiresAdmin: false,
    example:
      "/addMember (reply to user's message) or /addMember @username or /addMember CustomName",
  });

  registerCommand({
    name: "removeMember",
    description:
      "Remove a member from the team. Reply to their message or mention them.",
    parameters: [
      {
        name: "args",
        type: "string",
  ...
  registerCommand({
    ...
    parameters: [
      {
        ...
        required: true,
        description:
          "Updates in format: role: Developer, bio: Description, name: DisplayName",
      },
    ],
    ...
    requiresAdmin: false,
    example:
      "/updateMember role: Developer (reply to message) or /updateMember @username role: Designer",
  });

  registerCommand({
    name: "teamStats",
    description:
      "Show statistics about the team (total members, roles distribution, etc.).",
    parameters: [],
  ...
  registerCommand({
    name: "assign",
    description:
      "Assign tasks to team members. Tasks will be distributed evenly and randomly.",
    parameters: [
      {
        name: "tasks",
        type: "string",
  ...
  registerCommand({
    name: "taskStats",
    description:
      "Show task statistics (total, pending, completed, top contributor).",
    parameters: [],
  ...
  registerAIActionsCommands(registerCommand);
  ...
  registerCommand({
    ...
    handler: async (context, params) => {
      const { chatId } = context;
      const { sendMessage } = await import("@/utils/telegram");
      const { generateHelpMessage } = await import(
        "@/lib/services/commandRegistry"
      );
  ...
  registerCommand({
    ...
    handler: async (context, params) => {
      ...
      const configMessage =
        `⚙️ ${bold("Bot Configuration")} ${getRandomKaomoji()}\n\n` +
        `Version: 2.0.0\n` +
  ...
  registerCommand({
    name: "repoStats",
    description:
      "Show GitHub repository statistics (commits, contributions, etc.).",
    parameters: [
      {
        name: "period",
        type: "string",
