...
export async function detectCommandIntent(text, chatId) {
  ...
  const intentPatterns = [
    {
      patterns: [
        ...
        /(?:nuevo|new)\s+(?:miembro|member).*?@?(\w+)/i,
      ],
      command: "addMember",
      extractParams: (match) => {
    ...
    for (const pattern of intent.patterns) {
      ...
      if (match) {
        ...
        if (command) {
          ...
          console.log(`🎯 Intent detected: /${intent.command}`, "params:", params);
          return {
...
export async function handleAIDetectedCommand(message, detectedIntent) {
  ...
  try {
    ...
    console.error(`Error executing AI-detected command /${detectedIntent.command}:`, error);
...
export async function processCommand(message) {
  ...
  const detectedIntent = await detectCommandIntent(text, message.chat.id);
...
export async function isCommand(text) {
  ...
  const intent = await detectCommandIntent(text, null);
  return intent !== null;
}
