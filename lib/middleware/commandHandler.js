...
export async function handleDirectCommand(message) {
  ...
  try {
    console.log(
      `📝 Direct command: /${parsed.name} by user ${userId} in chat ${chatId}`,
    );
    console.log(`   Context:`, {
      chatId,
      userId,
...
export async function detectCommandIntent(text, chatId, message) {
  ...
  console.log(`🔍 Detecting intent in: "${text}"`);
  console.log(`   Lower case: "${lowerText}"`);
  ...
  const intentPatterns = [
    ...
    {
      patterns: [
        /(?:asigna|assign|asignar|distribuye|distribute)(?:\s+(?:la|las|el|los))?\s+(?:tarea|tareas|task|tasks|trabajo)(?:\s+de)?\s+(.+)/i,
        /(?:crea|crear|create)(?:\s+(?:la|las))?\s+(?:tarea|tareas|task|tasks)(?:\s+de)?\s+(.+)/i,
        /(?:agrega|agregar|add)(?:\s+(?:la|las))?\s+(?:tarea|tareas|task|tasks)(?:\s+de)?\s+(.+)/i,
      ],
      command: "assign",
      extractParams: (match) => {
  ...
  for (const intent of intentPatterns) {
    console.log(`   Testing intent: ${intent.command}`);
    for (const pattern of intent.patterns) {
      console.log(`      Pattern: ${pattern}`);
      const match = text.match(pattern);
      console.log(`      Match result:`, match ? `YES - "${match[0]}"` : "NO");
      if (match) {
        ...
        if (command) {
          ...
          console.log(`      ⚠️ Command ${intent.command} not registered`);
        }
      }
    }
  }
...
export async function handleAIDetectedCommand(message, detectedIntent) {
  ...
  try {
    console.log(
      `🤖 AI executing detected command: /${detectedIntent.command} in chat ${chatId}`,
    );
    console.log(`   Intent params:`, detectedIntent.params);
...
export async function processCommand(message) {
  ...
  console.log(
    `🔍 Processing message in chat ${chatId}: "${text.substring(0, 50)}..."`,
  );

  if (text.startsWith("/")) {
    console.log(`   → Direct command detected`);
    const result = await handleDirectCommand(message);
    if (result) {
      console.log(`   → Command handled:`, result.success ? "✅" : "❌");
      return result;
    }
  }

  console.log(`   → Checking for intent detection...`);
  ...
  if (detectedIntent) {
    console.log(`   → Intent found! Executing...`);
    const result = await handleAIDetectedCommand(message, detectedIntent);
    console.log(`   → Intent execution:`, result.success ? "✅" : "❌");
    return result;
  }

  console.log(`   → No command or intent detected, passing to AI`);
