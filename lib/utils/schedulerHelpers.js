...
export async function triggerRandomFunMessage() {
  const targetGroupIdStr = process.env.TARGET_GROUP_ID;
  if (!targetGroupIdStr) {
    console.warn("SCHEDULER: TARGET_GROUP_ID no configurado.");
    return;
  }
  ...
  try {
    ...
    await sendMessage(targetGroupId, messageToSend, "HTML");
