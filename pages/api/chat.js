...
import { executeIteratively } from "@/lib/ai/iterativeExecutor";
...
export async function getAiResponse(
  ...
  const {
    ...
    useIterative = true, // Enable iterative execution
  } = options;
  ...
  try {
    ...
    if (enableActions && chatId) {
      ...
      // Usar ejecución iterativa si está habilitada
      if (useIterative) {
        console.log("🔄 Using iterative execution...");

        const iterativeResult = await executeIteratively(
          aiResponse,
          context,
          systemPrompt,
          messages,
        ...
        return {
          message: iterativeResult.message,
          hasActions: iterativeResult.totalActionsExecuted > 0,
          executionResults: iterativeResult.allExecutionResults,
          success: iterativeResult.success,
          iterations: iterativeResult.iterations,
          totalActionsExecuted: iterativeResult.totalActionsExecuted,
          iterative: true,
        };
      }
